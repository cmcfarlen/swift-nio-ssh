...
public protocol NIOSSHClientUserAuthenticationInfo: Sendable {
    var sessionID: ByteBuffer { get }
    var availableMethods: NIOSSHAvailableUserAuthenticationMethods { get }
...
public protocol NIOSSHClientUserAuthenticationDelegate {
    ...
    ///
    /// - parameters:
    ///     - channel: The NIO Channel that is hosting the SSH handshake.
    ///     - availableMethods: The authentication methods the server is willing to accept.
    ///     - nextChallengePromise: An `EventLoopPromise` to be fulfilled with the next authentication offer.
    mutating func nextAuthenticationType(
...
extension NIOSSHClientUserAuthenticationDelegate {
    @available(*, deprecated)
    public mutating func nextAuthenticationType(
        authInfo: any NIOSSHClientUserAuthenticationInfo,
        nextChallengePromise promise: EventLoopPromise<NIOSSHUserAuthenticationOffer?>
    ) {
        self.nextAuthenticationType(availableMethods: authInfo.availableMethods, nextChallengePromise: promise)
    }
}
